...
const listaJuegos = () => {
  arrayJuegos = JSON.parse(localStorage.getItem("categoriasgames"));
...
const saveDataNewListLS = () => {
  localStorage.setItem("categoriasgames", JSON.stringify(arrayJuegos));
};
  ...
  arrayJuegos.forEach((element) => {
    ...
    row.innerHTML = `
          <div id="${element.id}">
            ...
            <td>${element.categoria}</td>
            <td>$ ${element.precio}</td>
            <td><a href=""></a><i class="fas fa-trash delete mx-2 "></i></td>
            <td><a href=""></a><i class="far fa-edit edit"></i></td>
          </div>
    `;
    lista.append(row);
  });
...
const destacados = arrayJuegos.find((juego) => juego.id == id);
...
const deleteNewListLS = (id) => {
  arrayJuegos = arrayJuegos.filter(juego => juego.id != id);
  saveDataNewListLS();
};

const editText = (titulo, precio, descripcion, categoria, thumbnailUrl) => {
  arrayJuegos = arrayJuegos.map((juego) => {
...
const editForm=(id)=>{
const element=arrayJuegos.find(juego=>juego.id==id)
